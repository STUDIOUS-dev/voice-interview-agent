...
The evaluator uses GPT-4o's `response_format: json_schema` to enforce a strict structured output on every evaluation turn:
...
Without `response_format: json_schema`, the LLM might:
...
The system uses a three-layer approach to support English, Hindi, and German interviews without switching any model or voice.

### Layer 1 — Speech-to-Text (Whisper)

```
Candidate speaks in Hindi → Whisper receives language="hi" explicitly
```

Passing the language code explicitly to the Whisper API prevents two problems:
1. **Misdetection:** Short technical answers (1-2 sentences) may be ambiguous to auto-detect. Explicit language prevents Whisper from transcribing Hindi as Bengali or German as Dutch.
2. **Technical term accuracy:** Words like "Python", "ORM", "REST", and "API" appear in all three languages unchanged. Whisper handles these correctly when the language context is set.

### Layer 2 — LLM Evaluation (GPT-4o)
...
### Layer 3 — Text-to-Speech (OpenAI TTS)

```
TTS input: Hindi text → alloy voice → Hindi audio output
```

The OpenAI TTS model auto-detects the language from the text content. No model switch or voice change is needed. The `alloy` voice produces natural-sounding output in all three supported languages.

### Translation Strategy

Questions are translated **once at session start** in a single batch LLM call:
...
| Whisper STT API | ~1.0s | Record-then-upload model; scales with audio length |
| GPT-4o evaluation | ~1.5–2.5s | Structured JSON output, ~200 token response |
| TTS generation API | ~1.0–2.0s | Scales with response length |
...
Replace the Whisper record-then-upload model with Deepgram's WebSocket streaming API:
...
- Deepgram Nova-2 matches Whisper accuracy on English technical speech and is within 5% on Hindi and German.

**Eliminates:** The 1.0s Whisper API latency and reduces the silence detection gap.

### Production Improvement 2: LLM Streaming + Sentence Chunking

Use `stream=True` on the GPT-4o chat completion:
...
All three OpenAI API calls (STT, TTS, LLM) use the same retry pattern:
```
Attempt 1 → failure → wait 2s
Attempt 2 → failure → wait 4s
Attempt 3 → failure → return error state / safe default
```
Exponential backoff respects rate limit windows without flooding the API with retries.

### JSON Parse Failure Fallback

If GPT-4o returns malformed JSON (rare with `json_schema` enforcement):
1. Attempt 1 fails → retry with an explicit instruction appended: *"Respond with ONLY the JSON object, no prose."*
2. Attempt 2 fails → return safe default: `{move_to_next: True, score: 3, spoken_reply: "Thank you for your answer. Let's move on.", ...}`
...
When pydub tries to decode the MP3 and ffmpeg is absent:
```python
except FileNotFoundError:
    os_name = platform.system()
    # Print exact install command for the detected OS
    sys.exit(1)
```
The exact install command for the user's OS is printed (not a generic error), reducing the support burden.

### Microphone Permission Handling

`calibrate_microphone()` catches `OSError` and `sr.RequestError`, prints clear instructions including the OS-specific settings path, and suggests `--mode text` as an immediate alternative. The program exits with code 1 rather than throwing a cryptic stack trace.

### Temp File Hygiene

All temporary files (`temp_input.wav`, `temp_output.mp3`) are deleted in `finally` blocks in both `stt.py` and `tts.py`. Files are guaranteed to be cleaned up even if an exception occurs during processing. This prevents accumulation of audio files that could contain PII (candidate speech data).
